...
.controller("relatorioRefeicaoCtrl", function($timeout, Analyze, Util, growl) {
    vm = this;
    
    vm.loader = false;
    vm.loaderBtn = false;
    vm.primeiraPesq = true;
    vm.loaderAnalitico = false;
    vm.refeicoes = [];
    vm.carregarMais = 0;
    vm.refeicaoAnalitico = {dobras: [], solicitacoes: [], trocas: []};
    vm.tipo_refeicao = [
    ...
    vm.fnPesquisarSintetico = function(params, skip_line) {
        ...
        if (params.dt_inicio.isValid() && params.dt_final.isValid()) {
            if (params.dt_final.isAfter(params.dt_inicio) || params.dt_final.isSame(params.dt_inicio, 'day')) {
                ...
                Analyze.publicSynthetic(params)
                    .then(function(resp) {                     
                        if (skip_line) {
    ...
    vm.fnPesquisarAnalitico = function(nr_refeicao) {
        vm.loaderAnalitico = true;

        $('#myModalRefAnalitico').modal('show');

        Analyze.publicAnalytical(nr_refeicao)
            .then(function(resp) {
                vm.refeicaoAnalitico = resp;
            })
            .catch(function(err) {
                Util.treatError(err);
            })
    ...
    vm.fnPrintModal = function() {
         $('.print-modal').html($('.modal-body').html());
         print();
    }
});
